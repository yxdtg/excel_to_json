...
function btnExcelToJson() {
    // 获取excel数据
    let excelData = document.getElementById("input-excelData").value;
    // 获取转换后的json数据
    document.getElementById("input-jsonData").value = excelToJson(excelData);
}
// 转换并复制
function btnExcelToJsonAndCopy() {
    btnExcelToJson();
    // 获取json数据
    let inputJsonData = document.getElementById("input-jsonData");
    // 选中并复制
...
function excelToJson(excelData) {
    ...
    // 获取类型列表
    let typeArr = JSON.parse(JSON.stringify(arr));
    ...
    // 获取值列表
    let valueArr = JSON.parse(JSON.stringify(arr));
